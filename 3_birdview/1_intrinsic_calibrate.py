...
for fname in images:
    ...
    if ret == True:
        ...
        for i in range(grid_corner.shape[0]):
            img = cv2.circle(img, (grid_corner[i, 0], grid_corner[i, 1]), radius=3, color=(0, 0, 255), thickness=1)
...
# np.savetxt('K_SVGA.csv', K, fmt='%.4f', delimiter=',')
# np.savetxt('D_SVGA.csv', D, fmt='%.4f', delimiter=',')